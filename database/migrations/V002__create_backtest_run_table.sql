...
CREATE TABLE quant.backtest_run(
    ...
    CONSTRAINT ck_backtest_run_strategy_name_not_blank CHECK(BTRIM(strategy_name)<>''),
    ...
    CONSTRAINT ck_backtest_run_timestamp_order CHECK(
                                                        (
                                                            started_at IS NULL 
...
CREATE INDEX idx_backtest_run_asset_created_at 
    ON quant.backtest_run(asset_id, created_at DESC);

CREATE INDEX idx_backtest_run_status_created_at 
    ON quant.backtest_run(status, created_at);

CREATE INDEX idx_backtest_run_strategy_created_at
    ON quant.backtest_run(strategy_name, created_at DESC);

CREATE TRIGGER trg_backtest_run_set_updated_at 
BEFORE UPDATE ON quant.backtest_run
FOR EACH ROW 
EXECUTE FUNCTION quant.set_updated_at();

COMMENT ON TABLE quant.backtest_run IS
    'Configuration and execution lifecycle of one strategy backtest.';

COMMENT ON COLUMN quant.backtest_run.public_id IS
    'Non-sequential identifier safe to expose through an external API.';

COMMENT ON COLUMN quant.backtest_run.asset_id IS
    'Canonical asset tested by this backtest run.';

COMMENT ON COLUMN quant.backtest_run.strategy_name IS
    'Stable machine-readable strategy identifier.';

COMMENT ON COLUMN quant.backtest_run.strategy_version IS
    'Version of the strategy implementation used for reproducibility.';

COMMENT ON COLUMN quant.backtest_run.parameters IS
    'Strategy-specific parameters stored as a JSON object.';

COMMENT ON COLUMN quant.backtest_run.engine_version IS
    'Application version or Git commit used to run the backtest.';

COMMENT ON COLUMN quant.backtest_run.status IS
    'Execution state: PENDING, RUNNING, COMPLETED or FAILED.';

COMMENT ON COLUMN quant.backtest_run.error_message IS
